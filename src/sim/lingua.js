...
export class LinguaOrganism {
  ...
  paramCount() {
    return this.V * this.E + this.K * this.E * this.H + this.H + this.H * this.V + this.V;
  }

  // One training step over a batch.
  // ctx: Int32Array [B * K] of char ids, tgt: Int32Array [B] next-char ids.
  // Returns the cross-entropy loss. This is the whole organism eating.
  trainBatch(ctx, tgt, B, stride = this.K) {
    const { V, E, K, H, emb, W1, b1, W2, b2, lr } = this;
    const D = K * E;
  ...
  predict(row) {
    const { V, E, K, H, emb, W1, b1, W2, b2 } = this;
    const D = K * E;
  ...
  growWidth(newH) {
    ...
    for (let j = 0; j < addH; j++) b1n[H + j] = gauss() * 0.01;
